...
server.post('/create-payment-intent', async (req, res) => {
  try {
    // Destructure and process the request body
    let { totalAmount, orderId } = req.body;

    // Ensure totalAmount is converted to the smallest currency unit
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      amount: totalAmount*100, // Pass amount in smallest unit (e.g., cents for USD)
      currency: 'inr',
      automatic_payment_methods: {
        enabled: true,
      },
      metadata: {
        orderId, // Attach orderId as metadata for reference
      },
    });

    console.log("PaymentIntent created successfully");

    // Respond with the client secret for client-side confirmation
    res.send({
...
// Catch errors for unhandled promise rejections
